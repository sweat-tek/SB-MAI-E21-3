...
public class SVGAttributeKeys extends AttributeKeys {
    ...
    private static class defultValues{
         final double DEFAULT_STROKE_WIDTH = 1d;
         final double DEFAULT_STROKE_MITER_LIMIT = 4d;
         final double DEFAULT_STROKE_DASH_PHASE = 0d;
    }   
    
    /** Sets SVG default values. */
    public static void setDefaults(Figure f) {
        defultValues defaults = new defultValues();
        // Fill properties
        // http://www.w3.org/TR/SVGMobile12/painting.html#FillProperties
        FILL_COLOR.basicSet(f, Color.black);
       // FILL_OPACITY.basicSet(f, 0d); /// MICHAL FLAGA 
        ...
        STROKE_WIDTH.basicSet(f, defaults.DEFAULT_STROKE_WIDTH);
        STROKE_CAP.basicSet(f, BasicStroke.CAP_BUTT);
        STROKE_JOIN.basicSet(f, BasicStroke.JOIN_MITER);
        STROKE_MITER_LIMIT.basicSet(f, defaults.DEFAULT_STROKE_MITER_LIMIT);
        IS_STROKE_MITER_LIMIT_FACTOR.basicSet(f, false);
        STROKE_DASHES.basicSet(f, null);
        STROKE_DASH_PHASE.basicSet(f, defaults.DEFAULT_STROKE_DASH_PHASE);
